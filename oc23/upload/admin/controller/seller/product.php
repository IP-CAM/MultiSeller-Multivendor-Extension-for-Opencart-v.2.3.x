...
class ControllersellerProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_seller_seller->addProduct($this->request->post);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_seller_seller->editProduct($this->request->get['product_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $product_id) {
				$this->model_seller_seller->deleteProduct($product_id);
			}
	...
	public function copy() {
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			foreach ($this->request->post['selected'] as $product_id) {
				$this->model_seller_seller->copyProduct($product_id);
			}
	...
	protected function getList() {
		...
		$product_total = $this->model_seller_seller->getTotalProductsSeller($filter_data);

		$results = $this->model_seller_seller->getProductsSeller($filter_data);
		...
		foreach ($results as $result) {
			...
			$product_specials = $this->model_seller_seller->getProductSpecials($result['product_id']);
	...
	protected function getForm() {
		...
		if (isset($this->request->post['product_recurrings'])) {
			...
			$data['product_recurrings'] = $this->model_catalog_recurring->getRecurrings($product_info['product_id']);
		} else {
	...
	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'seller/product')) {
		...
		if (utf8_strlen($this->request->post['keyword']) > 0) {
			$this->load->model('seller/url_alias');

			$url_alias_info = $this->model_seller_url_alias->getUrlAlias($this->request->post['keyword']);
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model']) || isset($this->request->get['filter_seller'])) {
			$this->load->model('seller/seller');
			$this->load->model('catalog/option');
			$this->load->model('catalog/product');
			...
			foreach ($results as $result) {
				...
				$json[] = array(
					...
					'seller'       => strip_tags(html_entity_decode($result['seller'], ENT_QUOTES, 'UTF-8')),
					'model'      => $result['model'],
					'option'     => $option_data,
					'price'      => $result['price']
				);
