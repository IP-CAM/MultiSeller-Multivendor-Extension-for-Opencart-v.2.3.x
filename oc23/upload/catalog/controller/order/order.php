...
class ControllerorderOrder extends Controller {
    ...
    public function delete() {
        ...
        if (isset($response['success'])) {
            ...
            $this->response->redirect($this->url->link('order/order', $url, true));
        }
    ...
    protected function getList()
    {
    	...
    	$data['invoice'] = $this->url->link('order/order/invoice', '', true);
        $data['shipping'] = $this->url->link('order/order/shipping', '', true);
        $data['insert'] = $this->url->link('sale/order/add', '', true);
        ...
        foreach ($results as $result) {
            $data['orders'][] = array(
                ...
                'invoice' => $this->url->link('order/order/invoice', 'order_id='.$result['order_id'].$url, true),
                ...
                'view' => $this->url->link('order/order/info', 'order_id='.$result['order_id'].$url, true),
                'add_to_transaction' => $this->url->link('order/order/add_to_transaction', 'seller_id='.$result['customer_id'].$url, true),
                'edit' => $this->url->link('sale/order/edit', 'order_id='.$result['order_id'].$url, true),
                'delete' => $this->url->link('order/order/delete', 'order_id='.$result['order_id'].$url, true),
            );
        }
        ...
        $data['sort_order'] = $this->url->link('order/order', 'sort=o.order_id'.$url, true);
        $data['sort_seller'] = $this->url->link('order/order', 'sort=seller'.$url, true);
        $data['sort_status'] = $this->url->link('order/order', 'sort=status'.$url, true);
        $data['sort_total'] = $this->url->link('order/order', 'sort=o.total'.$url, true);
        $data['sort_date_added'] = $this->url->link('order/order', 'sort=o.date_added'.$url, true);
        $data['sort_date_modified'] = $this->url->link('order/order', 'sort=o.date_modified'.$url, true);
        $data['sort_order_settlement'] = $this->url->link('order/order', 'sort=o.order_settlement'.$url, true);
        ...
        $pagination->url = $this->url->link('order/order', $url.'&page={page}', true);
    ...
    public function getForm() {
        ...
        $data['cancel'] = $this->url->link('order/order', $url, true);
    ...
    public function info()
    {
        ...
        if ($order_info && !empty($isorderseller)) {
            ...
            $data['shipping'] = $this->url->link('order/order/shipping', 'order_id='.(int) $this->request->get['order_id'], true);
            $data['invoice'] = $this->url->link('order/order/invoice', 'order_id='.(int) $this->request->get['order_id'], true);
            $data['edit'] = $this->url->link('sale/order/edit', 'order_id='.(int) $this->request->get['order_id'], true);
            $data['cancel'] = $this->url->link('order/order', $url, true);
          ...
          $data['seller_order_info'] = $this->model_order_order->getSellrOrdersettlement($this->request->get['order_id'], $this->customer->getId());
            ...
            if ($order_info['customer_id']) {
                $data['seller'] = $this->url->link('seller/seller/info','', true);
            } else {
                $data['seller'] = '';
            }
            ...
            foreach ($products as $product) {
                ...
                $data['products'][] = array(
                    ...
                    'href' => $this->url->link('sellerproduct/product/edit', 'product_id='.$product['product_id'], true),
                );
            ...
            foreach ($vouchers as $voucher) {
                $data['vouchers'][] = array(
                    ...
                    'href' => $this->url->link('seller/voucher/edit', 'voucher_id='.$voucher['voucher_id'], true),
                );
            }
    ...
    public function sellerhistory()
    {
        ...
        $pagination->page = $page;
        $pagination->limit = 10;
        $pagination->url = $this->url->link('order/order/sellerhistory', 'order_id='.$this->request->get['order_id'].'&page={page}', true);
