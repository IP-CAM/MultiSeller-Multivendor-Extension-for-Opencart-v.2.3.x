...
class ControllersellerproductProduct extends Controller
{
    ...
    public function add()
    {
        ...
        if (!$this->ProductLimit()) {
            $this->response->redirect($this->url->link('sellerproduct/product', '', true));
        //$this->getList();
        }
        $this->load->language('sellerproduct/product');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->response->redirect($this->url->link('sellerproduct/product', ''.$url, true));
        }
    ...
    protected function getList()
    {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('sellerproduct/product', ''.$url, true),
        );

        $data['insert'] = $this->url->link('sellerproduct/product/add', ''.$url, true);
        $data['delete'] = $this->url->link('sellerproduct/product/delete', ''.$url, true);
        ...
        $data['copy'] = $this->url->link('sellerproduct/product/copy',  ''.'&product_total='.$product_total.$url, true);
        ...
        foreach ($results as $result) {
            ...
            $data['products'][] = array(
                ...
                'edit' => $this->url->link('sellerproduct/product/edit', ''.'&product_id='.$result['product_id'].$url, true),
            );
        }
        ...
        $data['sort_name'] = $this->url->link('sellerproduct/product', ''.'&sort=pd.name'.$url, true);
        $data['sort_model'] = $this->url->link('sellerproduct/product', ''.'&sort=p.model'.$url, true);
        $data['sort_price'] = $this->url->link('sellerproduct/product', ''.'&sort=p.price'.$url, true);
        $data['sort_quantity'] = $this->url->link('sellerproduct/product', ''.'&sort=p.quantity'.$url, true);
        $data['sort_status'] = $this->url->link('sellerproduct/product', ''.'&sort=p.status'.$url, true);
        $data['sort_order'] = $this->url->link('sellerproduct/product', ''.'&sort=p.sort_order'.$url, true);
        ...
        $pagination->url = $this->url->link('sellerproduct/product', ''.$url.'&page={page}', true);
    ...
    protected function getForm()
    {
        ...
        if (!$isproductseller) {
            ...
            if (!isset($this->request->get['product_id'])) {
                $data['action'] = $this->url->link('sellerproduct/product/add', ''.$url, true);
            } else {
                $data['action'] = $this->url->link('sellerproduct/product/edit', ''.'&product_id='.$this->request->get['product_id'].$url, true);
            }

            $data['cancel'] = $this->url->link('sellerproduct/product', ''.$url, true);
